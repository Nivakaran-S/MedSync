...
import { Bot, ClipboardList, User, FileText, Video, Sparkles, RefreshCcw } from 'lucide-react';
import { appointmentApi, symptomApi } from '../services/api';
...
// #8 AI narrative summary — paragraph-form synthesis of the patient's record.
// Cached server-side for 24h; "Refresh" forces a regenerate.
function NarrativeCard() {
  const [data, setData] = useState<{ summary: string; generatedAt: string; cached: boolean } | null>(null);
  ...
  const load = async (refresh = false) => {
    setLoading(true);
    try {
      const r = await symptomApi.getNarrative({ refresh });
      setData({ summary: r.summary, generatedAt: r.generatedAt, cached: r.cached });
    } catch (err: any) {
      showToast(err.message || 'Could not generate narrative', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <h3 style={{ margin: 0, color: '#581c87' }}>AI health summary</h3>
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#7e22ce' }}>
              Plain-language summary of your records — generated by AI.
            </p>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          {!opened && (
            <button
              onClick={() => { setOpened(true); load(false); }}
      ...
      {opened && data && (
        <div style={{ marginTop: 14 }}>
          <p style={{ fontSize: '0.95rem', lineHeight: 1.6, color: '#3b0764', margin: 0, whiteSpace: 'pre-wrap' }}>
            {data.summary}
          </p>
          <p style={{ marginTop: 10, fontSize: '0.78rem', color: '#7e22ce' }}>
            {data.cached ? 'Cached' : 'Just generated'} · {new Date(data.generatedAt).toLocaleString()} · AI summary, not a clinical diagnosis.
          </p>
        </div>
      )}
    </div>
