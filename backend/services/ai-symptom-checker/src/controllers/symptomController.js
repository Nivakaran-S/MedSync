...
exports.analyzeSymptoms = async (req, res) => {
  try {
    ...
    // If the model's confidence is below the configured threshold, escalate
    // urgency to 'high' and tag the result so the UI can warn the user. The
    // threshold is configurable via env to allow tuning without a redeploy.
    ...
    const finalUrgency = isLowConfidence && urgencyOrder[rawUrgency] < urgencyOrder.high ? 'high' : rawUrgency;
    ...
    // Medium urgency → 48h follow-up nudge, high → 24h, low/emergency → none.
    const followUpAt = (() => {
    ...
    const disclaimers = [
      ...
      { type: 'model-info', text: `Generated by ${check.sourceModel}. AI models can make mistakes — consider getting a second opinion for serious symptoms.` },
    ...
    res.status(200).json({
      ...
      disclaimers,
      disclaimer: disclaimers[0].text, // legacy field, kept for backwards compatibility
    });
  } catch (error) {
...
// Patients can mark their own checks; doctors/admins can mark any.
exports.submitFeedback = async (req, res) => {
  try {
    ...
    // Upsert so a user can update their own feedback.
...
exports.getAdminAnalytics = async (req, res) => {
  try {
    ...
    const [totals, urgencyBreakdown, topSpecialties, dailyTrend, feedbackBreakdown, lowConfidenceCount] = await Promise.all([
      ...
      // A4: feedback breakdown — counts of false-positive / false-negative / correct
      SymptomCheckFeedback.aggregate([
        { $group: { _id: '$type', count: { $sum: 1 } } },
      ]),
      // A2: how many checks have been flagged as low confidence (last 30 days)
